...
def test_claude_full_access():
    ...
    try:
        ...
        print("⚠️  Claude CLI not found - install via: curl -fsSL https://claude.ai/cli | sh")
...
async def test_api_endpoints():
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print("❌ Cannot connect to API server - make sure it's running with 'make dev'")
...
def test_git_worktrees():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            result = subprocess.run([
                "git", "worktree", "add", str(worktree_path), "-b", "test/worktree-demo"
            ...
            if result.returncode == 0:
                ...
                subprocess.run(["git", "worktree", "remove", str(worktree_path)], 
                             capture_output=True)
                subprocess.run(["git", "branch", "-D", "test/worktree-demo"], 
                             capture_output=True)
