...
class CLIProcessManager:
    ...
    def _detect_prompt_ready(self, output: str) -> bool:
        ...
        if self.session_info.cli_tool == "claude":
            ...
            if "⏵⏵ bypass permissions" in output or "bypass permissions on" in output:
                logger.info(
            ...
            if ("╭─" in output and "│" in output) or ("╭──────────────────" in output):
                # Check if we have a substantial interface loaded
                buffer_check = (
                    self.current_output_buffer[-500:]
                    if len(self.current_output_buffer) > 500
                    else self.current_output_buffer
                )
                if "│" in buffer_check and len(buffer_check) > 100:
                    logger.info(
                        f"Session {self.session_info.session_id}: Detected Claude input box ready"
                    )
                    return True

            # Welcome screen with complete interface OR authenticated session
            if "Welcome to Claude Code!" in output:
                # For authenticated sessions, interface loads faster
                buffer_check = (
                    self.current_output_buffer[-1000:]
                    if len(self.current_output_buffer) > 1000
                    else self.current_output_buffer
                )
                # Simpler check - if we have the welcome message and box drawing characters
                if "│" in buffer_check or "╭─" in buffer_check:
                    logger.info(
                        f"Session {self.session_info.session_id}: Detected Claude welcome screen with interface"
                    )
                    return True

            # For authenticated sessions, Claude may just show the interface without "Welcome"
            # Check for a complete TUI interface in the buffer
            if len(self.current_output_buffer) > 200:
                recent_output = self.current_output_buffer[-500:]
                # Look for TUI box drawing chars that indicate interface is loaded
                has_box_top = "╭─" in recent_output or "┌─" in recent_output
                has_box_sides = "│" in recent_output
                has_box_bottom = "╰─" in recent_output or "└─" in recent_output

                if has_box_top and has_box_sides:
                    logger.info(
                        f"Session {self.session_info.session_id}: Detected Claude TUI interface structure"
                    )
                    return True
    ...
    def _detect_auth_patterns(self, output: str):
        ...
        if self.session_info.cli_tool == "claude":
            # Only mark as needing auth if we see the actual login prompt
            # Avoid false positives from regular output
            if "select login method" in output_lower or (
                "login" in output_lower and "browser" in output_lower
            ):
                logger.info(
                    f"Session {self.session_info.session_id}: Claude authentication prompt detected"
                )
                self.session_info.state = CLISessionState.WAITING_INPUT
                self.session_info.authentication_required = True
                self.session_info.auth_prompt = (
                    "Claude authentication required - please authenticate via CLI directly"
                )
                return  # Exit early to avoid conflicting state changes

            # Check for successful auth ONLY if we were waiting for auth
            if self.session_info.authentication_required and any(
    ...
    async def send_input(self, text: str, add_newline: bool = True):
        ...
        try:
            ...
            # For Claude, use a longer timeout as it may take time to process
            timeout = 300.0 if self.session_info.cli_tool == "claude" else 120.0
            try:
                ...
                logger.warning(
                    f"Session {self.session_info.session_id}: Command response timeout after {timeout}s"
                )
                self.session_info.state = CLISessionState.RUNNING  # Reset to running state
                await self._send_output(
                    f"\n[Response timeout after {timeout}s - CLI may still be processing]\n",
                    "stderr",
                )
...
class CLISessionManager:
    ...
    def __init__(self):
        ...
        # Session reuse for Claude to avoid repeated authentication
        self.claude_master_session: CLIProcessManager | None = None
        self.claude_master_session_id: str | None = None
    ...
    async def start_cli_process(self, session_id: str, full_access: bool = False) -> bool:
        ...
        # For Claude, try to reuse existing authenticated session
        if info.cli_tool == "claude" and full_access:
            if await self._try_reuse_claude_session(session_id):
                logger.info(f"Session {session_id}: Reusing existing authenticated Claude session")
                return True
        ...
        # Set up environment with proper auth inheritance
        ...
        try:
            ...
            # Mark as master session if it's the first authenticated Claude session
            if info.cli_tool == "claude" and full_access and not self.claude_master_session:
                self.claude_master_session = session
                self.claude_master_session_id = session_id
                logger.info(f"Session {session_id}: Set as Claude master session for reuse")
    ...
    async def _try_reuse_claude_session(self, session_id: str) -> bool:
        """Try to reuse an existing authenticated Claude session."""
        # Check if we have a master Claude session
        if not self.claude_master_session or not self.claude_master_session_id:
            return False

        # Check if master session is still running
        master_info = self.session_info.get(self.claude_master_session_id)
        if not master_info or master_info.state == CLISessionState.TERMINATED:
            # Master session is gone, clear reference
            self.claude_master_session = None
            self.claude_master_session_id = None
            return False

        # Check if master process is still alive
        if (
            not self.claude_master_session.process
            or self.claude_master_session.process.poll() is not None
        ):
            # Process died, clear reference
            self.claude_master_session = None
            self.claude_master_session_id = None
            return False

        # Reuse the master session for this new session
        logger.info(
            f"Session {session_id}: Reusing Claude master session {self.claude_master_session_id}"
        )

        # Point the new session to the master session's process manager
        self.sessions[session_id] = self.claude_master_session

        # Update session info to indicate it's running
        self.session_info[session_id].state = CLISessionState.RUNNING
        self.session_info[session_id].pid = self.claude_master_session.process.pid
...
""",
    ...
    def _get_cli_environment(self, cli_tool: str, full_access: bool) -> dict[str, str]:
        ...
        # CRITICAL: Ensure HOME is properly set so Claude can find ~/.claude/
        # This is the key to using your local computer's Claude settings!
        home_dir = os.path.expanduser("~")
        env["HOME"] = home_dir

        # Get USER safely - os.getlogin() can fail without a controlling terminal
        try:
            default_user = os.getlogin() if hasattr(os, "getlogin") else ""
        except (OSError, AttributeError):
            # Fallback to pwd module or environment variable
            try:
                import pwd
        ...
        env["USER"] = os.getenv("USER", default_user)
        ...
        if cli_tool == "claude":
            ...
            if api_key == "replace_me" or not api_key:
                # Remove to let Claude use its own auth from ~/.claude/
                env.pop("ANTHROPIC_API_KEY", None)

            # Explicitly set paths to ensure Claude finds its auth files
            claude_config_dir = os.path.join(home_dir, ".claude")
            if os.path.exists(claude_config_dir):
                env["CLAUDE_CONFIG_DIR"] = claude_config_dir
                # Also ensure XDG paths are set for Claude to find config
                env["XDG_CONFIG_HOME"] = os.path.join(home_dir, ".config")
                env["XDG_DATA_HOME"] = os.path.join(home_dir, ".local", "share")

                # Log for debugging
                credentials_file = os.path.join(claude_config_dir, ".credentials.json")
                if os.path.exists(credentials_file):
                    logger.info(f"Found Claude credentials at {credentials_file}")
                else:
                    logger.warning(f"No Claude credentials found at {credentials_file}")
    ...
    async def terminate_session(self, session_id: str, reason: str = "Manual termination") -> bool:
        ...
        try:
            ...
            # Check if this is the master Claude session
            if session_id == self.claude_master_session_id:
                # Clear master session reference
                self.claude_master_session = None
                self.claude_master_session_id = None
                logger.info(f"Cleared Claude master session reference for {session_id}")

            # Check if this session is reusing the master session
            if (
                self.claude_master_session
                and self.sessions.get(session_id) == self.claude_master_session
            ):
                # This is a reused session, just clean up references without terminating process
                logger.info(f"Session {session_id} was reusing master session, preserving process")
